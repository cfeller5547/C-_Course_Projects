...
#include <sys/stat.h>
 ...
 // Test if add correctly adds an element
    ...
    try {
        ...
        std::cout << "SUCCESS: bool add(int element) Correctly adds elements" <<
            std::endl;
    }
...
void test_get(SortableArray& x) {
    try {
        ...
        if (correct_val != test_val) {
            ...
            std::cout << "SUCCESS: int get(int index) Correctly gets value" <<
                std::endl;
        }
    }
    catch (std::exception& e) {
...
void test_save(SortableArray& x) {
    try {
        ...
        x.saveFile(filename);
        if (exists(filename)) {
            std::cout << "SUCCESS: bool saveFile(std::string fileName) Saves file"
                << std::endl;
...
void test_insertion_sort(SortableArray& x) {
    try {
        ...
        if (x.get(0) != 56 || x.get(999) != 99892) {
            ...
            std::cout << "SUCCESS: void insertionSort() Correctly sorts the file"
                << std::endl;
        }
    }
    catch (std::exception& e) {
...
void test_size(SortableArray& x, int size) {
    try {
        int returned_val = x.size();
        if (returned_val != size) {
            throw std::runtime_error("ERROR: int size() Incorrectly returned  size");
        }
        else {
            std::cout << "SUCCESS: int size() Correctly returned size" <<
                std::endl;
        }
    }
    catch (std::exception& e) {
...
// Test constructor given custom size of array
void test_constructor(SortableArray& x, int test_size) {
    try {
        int returned_val = x.size();
        if (returned_val != test_size) {
            throw std::runtime_error("ERROR: Constructor incorrectly sets size");
        }
        else {
            std::cout << "SUCCESS: Constructor correctly sets size" << std::endl;
        }
    }
    catch (std::exception& e) {
        std::cout << e.what() << std::endl;
...
int main() {
    ...
    test_remove(test1);
    test_size(test1, DEFAULT_TEST_SIZE);
    ...
    SortableArray test3(128);
    test_constructor(test3, 128);
    return 0;
}
